...
results_file = 'StepIndexWithTrench_' + time_string + '.csv' # MODIFY
...
a1 = np.linspace(3, 6, 5)
a2 = np.linspace(2, 3.5, 5)
a3 = np.linspace(2.5, 5, 5)
a4 = np.linspace(3, 5, 5)
a5 = np.linspace(15, 15, 1)

dop_a1 = np.linspace(0.04, 0.05, 5)
dop_a2 = np.linspace(0, 0.05, 5)
dop_a3 = np.linspace(0, 0.04, 5)
...
steps = len(a1) * len(a2) * len(a3) * len(a4) * len(dop_a1) * len(dop_a2) * len(dop_a3) # MODIFY
data_scan = np.zeros((steps, 3 * 9 + p))  # 9-> max output of fiber_profile.mode_data(), 3* because is three modes
i = 0
...
header = (
    ['a1(um)', 'a2(um)', 'a3(um)', 'a4(um)', 'a5(um)',
     ...
     "beta (Real) mode 1", "neff (Real) mode 1", "a_eff mode 1", "alpha mode 1", "dispersion mode 1", "isLeaky mode 1", "neffg mode 1", "fillFac mode 1", "gammaE mode 1",
     "beta (Real) mode 2", "neff (Real) mode 2", "a_eff mode 2", "alpha mode 2", "dispersion mode 2", "isLeaky mode 2", "neffg mode 2", "fillFac mode 2", "gammaE mode 2",
     "beta (Real) mode 3", "neff (Real) mode 3", "a_eff mode 3", "alpha mode 3", "dispersion mode 3", "isLeaky mode 3", "neffg mode 3", "fillFac mode 3", "gammaE mode 3",])
                                    ...
                                    for a5_dopant_val in dop_a5:
                                        for a1_alpha_val in alpha_a1:
                                            ...
                                            profile_type = ['Constant', 'Constant', 'Constant', 'Constant',
                                                            'Constant']
                                            materials = ['GeO2-SiO2', 'GeO2-SiO2', 'GeO2-SiO2',
                                                         'F-SiO2_2', 'SiO2']
                                            alphas = [a1_alpha_val, alpha_a2, alpha_a3, alpha_a4, alpha_a5]
                                            n_steps = 100 # 2 for constant
                                            ...
                                            data = np.hstack((data_mode1, data_mode2, data_mode3))
                                            ...
                                            print("Simulation goes for: " + str(
                                                100 * i / steps) + ' %' + f" It took: {elapsed_time} seconds")
